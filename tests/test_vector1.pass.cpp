...
#include <cstdlib>
#include "../new.h"
#include "../libstdc/malloc/memory.h"
...
{
    void *p = __STD_NAMESPACE::malloc(sz);
...
void count_construct(void *)
{ ++gCounter; }
void count_destruct(void *)
{ --gCounter; }
...
struct vec_on_stack
{
    void *storage;
    vec_on_stack()
        : storage(__cxxabiv1::__cxa_vec_new(10, 40, 8, throw_construct, throw_destruct))
    {}
...
{
    char heap[HEAP_SIZE];
    __STD_NAMESPACE::libstdc_allocator_initialize(heap, HEAP_SIZE);
    int retVal = 0;
    retVal += test_empty();
    retVal += test_counted();
#ifndef TEST_HAS_NO_EXCEPTIONS
